...
--
CREATE VIEW
  public."vwLocationsEntitiess" AS
SELECT
  p."peopleId" AS id,
  r."locationId",
  'people'::TEXT AS
...
FROM
  (
    (
      public."tblPeople" p
      JOIN public."brdgResidence" r USING ("peopleId")
    )
    JOIN public."tblLocations" l USING ("locationId")
  )
UNION
SELECT
  p."eventId" AS id,
...
FROM
  (
    (
      public."tblCallsEvents" p
      JOIN public."brdgVenue" r USING ("eventId")
    )
    JOIN public."tblLocations" l USING ("locationId")
  );
...
--
CREATE VIEW
  public."vwMediaEntities" AS (
    (
      (
        SELECT
          m."mediumId",
          p."presentationId" AS id,
          'presentations'::TEXT AS
        TYPE,
        m.rank
        FROM
          (
            public."brdgMediaPresentations" m
            JOIN public."tblPresentations" p USING ("presentationId")
          )
        UNION
        SELECT
          m."mediumId",
          p."publicationId" AS id,
          'publications'::TEXT AS
        TYPE,
        m.rank
        FROM
          (
            public."brdgMediaPublications" m
            JOIN public."tblPublications" p USING ("publicationId")
          )
      )
      UNION
      SELECT
      ...
      FROM
        (
          public."brdgMediaCourses" m
          JOIN public."tblCourses" p USING ("courseId")
        )
    )
    UNION
    SELECT
      m."mediumId",
      p."educMaterialId" AS id,
      'educmaterials'::TEXT AS
    TYPE,
    m.rank
    FROM
      (
        public."brdgMediaEducMaterials" m
        JOIN public."tblEducMaterials" p USING ("educMaterialId")
      )
  )
UNION
SELECT
...
FROM
  (
    public."brdgMediaTutorials" m
    JOIN public."tblTutorials" p USING ("tutorialId")
  );
...
SELECT
  ...
  d.size,
  d."sizeUnits",
  DATE_PART('year'::TEXT, d."startDate") AS startdate,
  DATE_PART('year'::TEXT, d."endDate") AS enddate,
  d.description,
  t."fileName",
  m."fileNameURL",
  d.type
FROM
  (
    (
      public."tblDatasets" d
      LEFT JOIN (
        SELECT
          "brdgThumbsDatasets"."thumbId",
          "brdgThumbsDatasets"."thumbDatasetId",
          "brdgThumbsDatasets"."datasetId",
          "brdgThumbsDatasets".rank,
          "tblThumbs"."fileName"
        FROM
          (
            public."brdgThumbsDatasets"
            JOIN public."tblThumbs" USING ("thumbId")
          )
        WHERE
          ("brdgThumbsDatasets".rank = 1)
      ) t USING ("datasetId")
    )
    LEFT JOIN (
      SELECT
        "brdgMediaDataset"."mediumId",
        "brdgMediaDataset"."mediumDatasetId",
        "brdgMediaDataset"."datasetId",
        "brdgMediaDataset".rank,
        "tblMedia"."fileNameURL",
        "tblMedia".type
      FROM
        (
          public."brdgMediaDataset"
          JOIN public."tblMedia" USING ("mediumId")
        )
      WHERE
        ("brdgMediaDataset".rank = 1)
    ) m USING ("datasetId")
  );
...
FROM
  (
    (
      public."brdgMediaCallsEvents" b
      JOIN public."tblMedia" m USING ("mediumId")
    )
    JOIN public."tblCallsEvents" e USING ("eventId")
  )
WHERE
  (b.rank = 1);
...
FROM
  (
    public."brdgPeopleContactInformation" b
    JOIN public."tblContactInformation" t USING ("contactInformationId")
  )
WHERE
  (
    ((t."contactType")::TEXT = '0001'::TEXT)
    AND (b.rank = 1)
  );
...
--
CREATE VIEW
  public."vwPeopleBase" AS
SELECT
  p."peopleId",
  public."fncFmtName" ((p."fName")::TEXT, (p."mName")::TEXT, (p."lName")::TEXT) AS NAME,
  hp."contactItem" AS homepage
FROM
  (
    (
      public."tblPeople" p
...
--
CREATE VIEW
  public."vwPresentationEventPresenters" AS
SELECT
  p."presentationEventId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(b.name, ''::TEXT) || '###'::TEXT) || (COALESCE(b.homepage, ''::CHARACTER VARYING))::TEXT)),
    '@@@'::TEXT
  ) AS presenters
FROM
  (
    public."brdgPresenters" p
    LEFT JOIN public."vwPeopleBase" b USING ("peopleId")
  )
GROUP BY
  p."presentationEventId";
...
--
CREATE VIEW
  ivl."vwIVLPresentationsAll" AS
SELECT DISTINCT
  p."presentationId",
  p.title AS presentationtitle,
  p."additionalInfo",
  p.type AS presentationtype,
  DATE_PART('year'::TEXT, pe.date) AS date,
  pe.date AS exactdate,
  ev.title AS eventstitle,
  ev."fileNameURL" AS eventurl,
  pep.presenters,
  l.city,
  l."StateProvince",
  lo."Country",
      ...
      (
        (
          (
            (
              (
                public."tblPresentations" p
                LEFT JOIN public."brdgPresentationsEvents" pe USING ("presentationId")
              )
              LEFT JOIN public."vwEventsURL" ev USING ("eventId")
            )
            LEFT JOIN public."vwPresentationEventPresenters" pep USING ("presentationEventId")
          )
          LEFT JOIN public."fncGetLocations" ('events'::TEXT) l (
            id,
            "locationId",
            TYPE,
            city,
        ...
        LEFT JOIN (
          SELECT
            "brdgVenue"."locationId",
            "brdgVenue"."eventLocationId",
            "brdgVenue"."eventId",
            "tblLocations".city,
            "tblLocations"."zipCode",
            "tblLocations"."latDD",
            "tblLocations"."longDD",
            "tblLocations"."addrLine1",
            "tblLocations"."addrLine2",
            "tblLocations"."Country",
            "tblLocations"."StateProvince",
            "tblLocations"."descriptionString"
          FROM
            (
              public."brdgVenue"
              JOIN public."tblLocations" USING ("locationId")
            )
        ) lo USING ("eventId")
      )
      LEFT JOIN (
        SELECT
          "fncGetMedia"."mediumId",
          "fncGetMedia".id,
          "fncGetMedia".type,
          "fncGetMedia".rank
        FROM
          public."fncGetMedia" ('presentations'::TEXT) "fncGetMedia" (
            "mediumId",
            id,
            TYPE,
            RANK
        ...
        WHERE
          ("fncGetMedia".rank = 1)
      ) m ON ((p."presentationId" = m.id))
    )
    LEFT JOIN public."tblMedia" t USING ("mediumId")
  )
WHERE
  (
    ((p.type)::TEXT ~~ '10'::TEXT)
    OR ((p.type)::TEXT ~~ '100'::TEXT)
  )
ORDER BY
  pe.date DESC,
  p."presentationId",
...
--
CREATE VIEW
  public."vwEventAttendees" AS
SELECT
  e."eventId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name) || '###'::TEXT) || (COALESCE(p.homepage, ''::CHARACTER VARYING))::TEXT)),
    '@@@'::TEXT
  ) AS attendees
FROM
  (
    (
      public."tblCallsEvents" e
      LEFT JOIN public."brdgOrganizersAttendees" o USING ("eventId")
    )
    LEFT JOIN public."vwPeopleBase" p USING ("peopleId")
  )
WHERE
  ((o.type)::TEXT ~~ 'a'::TEXT)
GROUP BY
...
--
CREATE VIEW
  public."vwEventsPresentations" AS
SELECT
  p."eventId",
  p."presentationId",
  p.date,
...
FROM
  (
    public."brdgPresentationsEvents" p
    JOIN public."vwEventsURL" u USING ("eventId")
  );
...
SELECT DISTINCT
  ...
  e."endDate",
  e.description,
  public."fncFmtEventType" ((e.type)::TEXT) AS eventtype,
  o.organizers,
  a.attendees,
  s."fileNameURL",
  s.type AS filetype,
  l.city,
  l."StateProvince",
  DATE_PART('year'::TEXT, e."startDate") AS YEAR,
  l."Country",
  pr."presentationId",
  r.presentationtitle AS pretitle,
  r."fileNameURL" AS prefilenameurl,
  r.presenters,
  public."fncGetEventOrder" ((e.type)::TEXT) AS typeorder
FROM
  (
    (
      (
        (
          (
            (
              public."tblCallsEvents" e
              LEFT JOIN public."vwEventOrganizers" o USING ("eventId")
            )
            LEFT JOIN public."vwEventAttendees" a USING ("eventId")
          )
          LEFT JOIN (
            SELECT
              b."mediumId",
            ...
            FROM
              (
                public."brdgMediaCallsEvents" b
                JOIN public."tblMedia" me USING ("mediumId")
              )
            WHERE
              (b.rank = 1)
          ) s USING ("eventId")
      ...
      LEFT JOIN public."vwEventsPresentations" pr USING ("eventId")
    )
    LEFT JOIN ivl."vwIVLPresentationsAll" r USING ("presentationId")
  )
ORDER BY
  e."endDate" DESC,
  e."eventId",
  ...
  e."startDate",
  e.description,
  public."fncFmtEventType" ((e.type)::TEXT),
  o.organizers,
  a.attendees,
  s."fileNameURL",
  s.type,
...
FROM
  (
    (
      public."brdgCoPIs" c
      JOIN public."vwPeopleBase" b USING ("peopleId")
    )
    JOIN public."tblGrants" t USING ("grantId")
  )
GROUP BY
  c."grantId";
...
--
CREATE VIEW
  public."vwGrantsResearch" AS
SELECT
  c."grantId",
  ARRAY_TO_STRING(
    public.array_accum ((((COALESCE(b."shortTitle", ''::CHARACTER VARYING))::TEXT || '###'::TEXT) || (b."researchId")::TEXT)),
    '@@@'::TEXT
  ) AS "researchGrants"
FROM
  (
    (
      public."brdgFunding" c
      JOIN public."tblResearch" b USING ("researchId")
    )
    JOIN public."tblGrants" t USING ("grantId")
  )
GROUP BY
...
--
CREATE VIEW
  ivl."vwIVLFundingGrantsCurrent" AS
SELECT
  g."grantId",
  g."grantName",
  TO_CHAR(g.amount, '999999999'::TEXT) AS amount,
  g."grantTitle",
  g."startDate",
  g."endDate",
  c."copisGrants",
...
FROM
  (
    (
      (
        public."tblGrants" g
        LEFT JOIN public."vwGrantsCopis" c USING ("grantId")
      )
      LEFT JOIN public."vwGrantsResearch" d USING ("grantId")
    )
    LEFT JOIN public."tblOrganizations" o USING ("organizationId")
  )
WHERE
  (
    (g."startDate" < NOW())
...
--
CREATE VIEW
  ivl."vwIVLFundingGrantsPast" AS
SELECT
  g."grantId",
  g."grantName",
  TO_CHAR(g.amount, '999,999,999'::TEXT) AS amount,
  g."grantTitle",
  g."startDate",
  g."endDate",
  c."copisGrants",
...
--
CREATE VIEW
  ivl."vwIVLFundingGrantsPending" AS
SELECT
  g."grantId",
  TO_CHAR(g.amount, '999,999,999'::TEXT) AS amount,
  g."grantTitle",
...
FROM
  (
    public."tblHardware" d
    LEFT JOIN (
      SELECT
        "brdgThumbsHardware"."thumbId",
        "brdgThumbsHardware"."thumbHardwareId",
        "brdgThumbsHardware"."hardwareId",
        "brdgThumbsHardware".rank,
        "tblThumbs"."fileName"
      FROM
        (
          public."brdgThumbsHardware"
          JOIN public."tblThumbs" USING ("thumbId")
        )
      WHERE
        ("brdgThumbsHardware".rank = 1)
    ) s USING ("hardwareId")
  );
...
--
CREATE VIEW
  ivl."vwIVLHomepage" AS
SELECT
  "tblHomepage".id,
  "tblHomepage".content_type,
  "tblHomepage".caption,
  "tblHomepage".link,
  "tblHomepage".image,
  "tblHomepage".image_alt,
  "tblHomepage".date_added,
  "tblHomepage".panel,
  "tblHomepage".display_order
FROM
  public."tblHomepage";
...
--
CREATE VIEW
  ivl."vwIVLNetscitalks" AS
SELECT
  "tblNetscitalks".id,
  "tblNetscitalks".guest,
  "tblNetscitalks".homepage,
  "tblNetscitalks".title,
  "tblNetscitalks".date,
  "tblNetscitalks".institution,
  "tblNetscitalks".image_url,
  "tblNetscitalks".slide_url,
  "tblNetscitalks".video_url,
  "tblNetscitalks".abstract,
  "tblNetscitalks".bio,
  "tblNetscitalks"."time",
  "tblNetscitalks".location
FROM
  public."tblNetscitalks";
...
--
CREATE VIEW
  public."vwCollabsCurrent" AS
SELECT DISTINCT
  b."peopleId"
FROM
  public."brdgTeamCollabs" b
WHERE
  (
    (
      (b."endDate" IS NULL)
...
--
CREATE VIEW
  public."vwCollabsResearch" AS
SELECT
  b."peopleId",
  ARRAY_TO_STRING(public.array_accum ((((t."shortTitle")::TEXT || '###'::TEXT) || (t."researchId")::TEXT)), '@@@'::TEXT) AS "researchCollabs",
  public.array_accum (t."startDate") AS "startDate",
  public.array_accum (t."endDate") AS "endDate"
FROM
  (
    public."brdgTeamCollabs" b
    JOIN public."tblResearch" t USING ("researchId")
  )
WHERE
  ((b.type)::TEXT ~~ '%001%'::TEXT)
GROUP BY
  b."peopleId";
...
FROM
  (
    public."tblThumbs" t
    JOIN public."brdgThumbsPeople" b USING ("thumbId")
  )
WHERE
  ((b.rank)::TEXT ~~ '%1%'::TEXT)
UNION
...
FROM
  (
    public."tblThumbs" t
    JOIN public."brdgThumbsDatasets" b USING ("thumbId")
  )
WHERE
  ((b.rank)::TEXT ~~ '%1%'::TEXT);
...
--
CREATE VIEW
  public."vwWebPeopleBase" AS
SELECT
  b."peopleId",
  b.name,
  b.homepage,
  pt."fileName",
  em."contactItem" AS email
FROM
  (
    (
      public."vwPeopleBase" b
      LEFT JOIN (
        SELECT
          t."thumbId",
          t.id,
          ...
          t.type
        FROM
          public."vwPrimaryThumbs" t
        WHERE
          (t.type ~~ '%people%'::TEXT)
      ) pt ON ((b."peopleId" = pt.id))
    )
    LEFT JOIN public."vwPrimaryEmails" em USING ("peopleId")
  );
...
--
CREATE VIEW
  ivl."vwIVLPeopleCollabsCurrent" AS
SELECT
  "vwWebPeopleBase"."peopleId",
  "vwWebPeopleBase".name,
  "vwWebPeopleBase".homepage,
  "vwWebPeopleBase"."fileName",
  "vwWebPeopleBase".email,
  "vwCollabsResearch"."researchCollabs",
  "vwCollabsResearch"."startDate",
  "vwCollabsResearch"."endDate"
FROM
  (
    (
      public."vwWebPeopleBase"
      JOIN public."vwCollabsCurrent" USING ("peopleId")
    )
    LEFT JOIN public."vwCollabsResearch" USING ("peopleId")
  );
...
--
CREATE VIEW
  ivl."vwIVLPeopleCollabsPrevious" AS
SELECT
  "vwWebPeopleBase"."peopleId",
  "vwWebPeopleBase".name,
  "vwWebPeopleBase".homepage,
...
FROM
  (
    (
      public."vwWebPeopleBase"
      JOIN public."vwCollabsPrevious" USING ("peopleId")
    )
    LEFT JOIN public."vwCollabsResearch" USING ("peopleId")
  );
...
--
CREATE VIEW
  public."vwIndepStudiesCurrent" AS
SELECT
  i."indepStudyId",
  i.topic,
  i.description,
  a."indepStudentSupervisorId",
  a."peopleId",
  a.type,
  a.semester,
  a.year,
...
FROM
  (
    public."tblIndepStudies" i
    JOIN (
      SELECT
        "brdgIndepStudentsSupervisors"."indepStudentSupervisorId",
        "brdgIndepStudentsSupervisors"."peopleId",
        "brdgIndepStudentsSupervisors"."indepStudyId",
        "brdgIndepStudentsSupervisors".type,
        "brdgIndepStudentsSupervisors".semester,
        "brdgIndepStudentsSupervisors".year,
        "brdgIndepStudentsSupervisors"."courseNumber"
      FROM
        public."brdgIndepStudentsSupervisors"
      WHERE
        ("brdgIndepStudentsSupervisors".type = 10)
    ) a USING ("indepStudyId")
  )
WHERE
  (
    (a.year > NOW())
...
--
CREATE VIEW
  ivl."vwIVLPeopleIndepStudentsCurrent" AS
SELECT
  p."peopleId",
  p.name,
  p.homepage,
...
FROM
  (
    public."vwWebPeopleBase" p
    JOIN public."vwIndepStudiesCurrent" i USING ("peopleId")
  )
ORDER BY
  i.year DESC,
  i.semester DESC;
...
--
CREATE VIEW
  public."vwIndepStudiesPrevious" AS
SELECT
  i."indepStudyId",
  i.topic,
  i.description,
...
--
CREATE VIEW
  ivl."vwIVLPeopleIndepStudentsPrevious" AS
SELECT
  p."peopleId",
  p.name,
  p.homepage,
...
FROM
  (
    public."vwWebPeopleBase" p
    JOIN public."vwIndepStudiesPrevious" i USING ("peopleId")
  )
ORDER BY
  i.year DESC,
  i.semester DESC;
  ...
  (
    ...
    JOIN (
      SELECT
        ...
        sc."degreeType"
      FROM
        public."vwStudentsCurrent" sc
      WHERE
        (sc."degreeType" = 'm'::bpchar)
    ) s USING ("peopleId")
  );
...
--
CREATE VIEW
  public."vwAffiliations" AS
SELECT
  a."peopleId",
  o."organizationName"
FROM
  (
    public."brdgAffiliations" a
    JOIN public."tblOrganizations" o USING ("organizationId")
  );
...
--
CREATE VIEW
  public."vwTeamCurrentResearch" AS
SELECT
  b."peopleId",
  ARRAY_TO_STRING(public.array_accum ((((t."shortTitle")::TEXT || '###'::TEXT) || (t."researchId")::TEXT)), '@@@'::TEXT) AS "researchTeam"
FROM
  (
    public."brdgTeamCollabs" b
    JOIN public."tblResearch" t USING ("researchId")
...
--
CREATE VIEW
  ivl."vwIVLPeopleMembersCurrent" AS
SELECT
  wb."peopleId",
  wb.name,
  wb.homepage,
  ...
  c."startDate",
  c."endDate",
  DATE_PART('year'::TEXT, c."startDate") AS startyear,
  DATE_PART('month'::TEXT, c."startDate") AS startmonth,
  DATE_PART('day'::TEXT, c."startDate") AS startday,
  DATE_PART('year'::TEXT, c."endDate") AS endyear,
  DATE_PART('month'::TEXT, c."endDate") AS endmonth,
  DATE_PART('day'::TEXT, c."endDate") AS endday,
  a."organizationName" AS affiliations,
  c.display_order,
  c.phone,
  c.fax,
  c.office,
...
FROM
  (
    (
      (
        public."vwWebPeopleBase" wb
        JOIN public."vwMembersCurrent" c USING ("peopleId")
      )
      LEFT JOIN public."vwTeamCurrentResearch" r USING ("peopleId")
    )
    LEFT JOIN public."vwAffiliations" a USING ("peopleId")
  );
...
--
CREATE VIEW
  public."vwMembersPrevious" AS
SELECT
  s."peopleId",
  "tblCenterMembers"."centerMemberId",
  "tblCenterMembers"."startDate",
  "tblCenterMembers"."endDate",
  "tblCenterMembers".ivl,
  "tblCenterMembers".cns,
  "tblCenterMembers"."jobDescription"
FROM
  (
    (
      (
      ...
      SELECT
        c."peopleId"
      FROM
        public."vwMembersCurrent" c
    ) s
    LEFT JOIN public."tblCenterMembers" USING ("peopleId")
  );
...
--
CREATE VIEW
  ivl."vwIVLPeopleMembersPrevious" AS
SELECT
  wb."peopleId",
  wb.name,
  wb.homepage,
...
FROM
  (
    (
      public."vwWebPeopleBase" wb
      JOIN public."vwMembersPrevious" c USING ("peopleId")
    )
    LEFT JOIN public."vwTeamResearch" r USING ("peopleId")
  )
ORDER BY
  wb.name,
  c."startDate" DESC;
...
--
CREATE VIEW
  ivl."vwIVLPeoplePhDsPrevious" AS
SELECT
  b."peopleId",
  b.name,
  b.homepage,
...
--
CREATE VIEW
  ivl."vwIVLPeopleStudentsCurrent" AS
SELECT
  b."peopleId",
  b.name,
  b.homepage,
...
--
CREATE VIEW
  ivl."vwIVLPeopleStudentsPrevious" AS
SELECT
  b."peopleId",
  b.name,
  b.homepage,
...
--
CREATE VIEW
  public."vwPublicationAuthors" AS
SELECT
  p."publicationId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(b.name, ''::TEXT) || '###'::TEXT) || (COALESCE(b.homepage, ''::CHARACTER VARYING))::TEXT)),
  ...
  (
    (
      ...
      FROM
        (
          public."vwPeopleBase" b
          JOIN public."vwAuthorsRanked" a USING ("peopleId")
        )
      ORDER BY
        a.rank
    ) b
...
--
CREATE VIEW
  public."vwPublicationEditors" AS
SELECT
  z."publicationId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(z.name, ''::TEXT) || '###'::TEXT) || (COALESCE(z.homepage, ''::CHARACTER VARYING))::TEXT)),
    '@@@'::TEXT
  ) AS editors
FROM
  (
    ...
    FROM
      (
        (
          ...
          JOIN (
            SELECT
              "brdgAuthorsEditors"."authorEditorId",
              "brdgAuthorsEditors"."peopleId",
              "brdgAuthorsEditors"."publicationId",
              "brdgAuthorsEditors".rank,
              "brdgAuthorsEditors".type
            FROM
              public."brdgAuthorsEditors"
            WHERE
              ("brdgAuthorsEditors".type ~~ 'e'::TEXT)
            ORDER BY
              "brdgAuthorsEditors".rank
          ) a USING ("peopleId")
        )
...
--
CREATE VIEW
  ivl."vwIVLPublicationsAll" AS
SELECT
  p."publicationId",
  p."publicationTitle",
  DATE_PART('YEAR'::TEXT, p."publicationDate") AS date,
  p."publicationVenue",
  p."venueVolume",
  p."venueNumber",
  p."venuePages",
  public."fncFmtPubType" ((p.type)::TEXT) AS
TYPE,
p.publisher,
p."venueChapter",
a.authors,
...
t."fileNameURL",
t.type AS linktype,
public."fncGetPubOrder" ((p.type)::TEXT) AS typeorder,
TO_CHAR((p."publicationDate")::TIMESTAMP WITH TIME ZONE, 'YYYY'::TEXT) AS YEAR
FROM
  (
    (
      (
        (
          public."tblPublications" p
          LEFT JOIN public."vwPublicationAuthors" a USING ("publicationId")
        )
        LEFT JOIN public."vwPublicationEditors" e USING ("publicationId")
      )
      LEFT JOIN (
        SELECT
          "fncGetMedia"."mediumId",
          ...
          "fncGetMedia".rank
        FROM
          public."fncGetMedia" ('publications'::TEXT) "fncGetMedia" (
            "mediumId",
            id,
            TYPE,
            RANK
...
--
CREATE VIEW
  public."vwResearchGrants" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(public.array_accum (p."grantName"), '@@@'::TEXT) AS grants
FROM
  (
    public."brdgFunding" b
    JOIN public."tblGrants" p USING ("grantId")
  )
GROUP BY
  b."researchId";
...
--
CREATE VIEW
  public."vwResearchPublications" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (
                                        ...
                                        (
                                          (
                                            (
                                              (
                                                (
                                                  ((COALESCE(p."publicationTitle", ''::CHARACTER VARYING))::TEXT || '%%%'::TEXT) || COALESCE((DATE_PART('year'::TEXT, p."publicationDate"))::TEXT, ''::TEXT)
                                                ) || '%%%'::TEXT
                                              ) || (COALESCE(p."publicationVenue", ''::CHARACTER VARYING))::TEXT
                                            ) || '%%%'::TEXT
                                          ) || (COALESCE(p."publicationVenue", ''::CHARACTER VARYING))::TEXT
                                        ) || '%%%'::TEXT
                                      ) || (COALESCE(p."venueVolume", ''::CHARACTER VARYING))::TEXT
                                    ) || '%%%'::TEXT
                                  ) || (COALESCE(p."venueNumber", ''::CHARACTER VARYING))::TEXT
                                ) || '%%%'::TEXT
                              ) || (COALESCE(p."venuePages", ''::CHARACTER VARYING))::TEXT
                            ) || '%%%'::TEXT
                          ) || (COALESCE(p.type, ''::CHARACTER VARYING))::TEXT
                        ) || '%%%'::TEXT
                      ) || (COALESCE(p.publisher, ''::CHARACTER VARYING))::TEXT
                    ) || '%%%'::TEXT
                  ) || (COALESCE(p."venueChapter", ''::CHARACTER VARYING))::TEXT
                ) || '%%%'::TEXT
              ) || COALESCE(a.authors, ''::TEXT)
            ) || '%%%'::TEXT
          ) || COALESCE(e.editors, ''::TEXT)
        ) || '%%%'::TEXT
      )
    ),
    '***'::TEXT
...
FROM
  (
    (
      (
        public."brdgResearchPublications" b
        JOIN public."tblPublications" p USING ("publicationId")
      )
      LEFT JOIN public."vwPublicationAuthors" a USING ("publicationId")
    )
...
--
CREATE VIEW
  public."vwResearchTeamCollabsCurrent" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name, ''::TEXT) || '###'::TEXT) || COALESCE((p.homepage)::TEXT, ''::TEXT))),
    '@@@'::TEXT
  ) AS collabscurrent
FROM
  (
    public."brdgTeamCollabs" b
    JOIN public."vwWebPeopleBase" p USING ("peopleId")
  )
WHERE
  (
    ((b.type)::TEXT = '001'::TEXT)
...
--
CREATE VIEW
  public."vwResearchTeamCollabsPast" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name, ''::TEXT) || '###'::TEXT) || COALESCE((p.homepage)::TEXT, ''::TEXT))),
...
--
CREATE VIEW
  public."vwResearchTeamLeadsCurrent" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name, ''::TEXT) || '###'::TEXT) || COALESCE((p.homepage)::TEXT, ''::TEXT))),
...
--
CREATE VIEW
  public."vwResearchTeamMembersCurrent" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name, ''::TEXT) || '###'::TEXT) || COALESCE((p.homepage)::TEXT, ''::TEXT))),
...
--
CREATE VIEW
  public."vwResearchTeamMembersPast" AS
SELECT
  b."researchId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(p.name, ''::TEXT) || '###'::TEXT) || COALESCE((p.homepage)::TEXT, ''::TEXT))),
        ...
        (
          (
            (
              (
                (
                  public."tblResearch" r
                  LEFT JOIN public."vwResearchGrants" g USING ("researchId")
                )
                LEFT JOIN public."vwResearchTeamLeadsCurrent" l USING ("researchId")
              )
              LEFT JOIN public."vwResearchTeamMembersCurrent" mc USING ("researchId")
            )
            LEFT JOIN public."vwResearchTeamMembersPast" mp USING ("researchId")
          )
          LEFT JOIN public."vwResearchTeamCollabsCurrent" cc USING ("researchId")
        )
        LEFT JOIN public."vwResearchTeamCollabsPast" cp USING ("researchId")
      )
      LEFT JOIN public."vwResearchPublications" p USING ("researchId")
    )
    LEFT JOIN (
      SELECT
        "tblThumbs"."fileName",
        "brdgThumbsResearch"."researchId"
      FROM
        (
          public."tblThumbs"
          JOIN public."brdgThumbsResearch" USING ("thumbId")
        )
      WHERE
        ("brdgThumbsResearch".rank = 1)
    ) t USING ("researchId")
  )
WHERE
  (
...
--
CREATE VIEW
  ivl."vwIVLSoftwareAll" AS
SELECT
  s."softwareId",
  s."softwareTitle",
  s."softwareDescription",
  t."fileName",
  m."fileNameURL",
  st.label AS "softwareType"
FROM
  (
    (
      (
        public."tblSoftware" s
        LEFT JOIN (
          SELECT
            "brdgThumbsSoftware"."thumbId",
            "brdgThumbsSoftware"."thumbSoftwareId",
            "brdgThumbsSoftware"."softwareId",
            "brdgThumbsSoftware".rank,
            "tblThumbs"."fileName"
          FROM
            (
              public."brdgThumbsSoftware"
              JOIN public."tblThumbs" USING ("thumbId")
            )
          WHERE
            ("brdgThumbsSoftware".rank = 1)
        ) t USING ("softwareId")
      )
      LEFT JOIN (
        SELECT
          "brdgMediaSoftware"."mediumId",
          "brdgMediaSoftware"."mediumSoftwareId",
          "brdgMediaSoftware"."softwareId",
          "brdgMediaSoftware".rank,
          "tblMedia"."fileNameURL",
          "tblMedia".type
        FROM
          (
            public."brdgMediaSoftware"
            JOIN public."tblMedia" USING ("mediumId")
          )
        WHERE
          ("brdgMediaSoftware".rank = 1)
      ) m USING ("softwareId")
    )
    LEFT JOIN public."tblSoftwareType" st USING ("softwareTypeID")
  )
ORDER BY
  st."sortOrder";
...
--
CREATE VIEW
  ivl."vwIVLSoftwareType" AS
SELECT
  "tblSoftwareType"."softwareTypeID",
  "tblSoftwareType".label,
  "tblSoftwareType"."sortOrder"
FROM
...
--
CREATE VIEW
  ivl."vwIVLStaticPages" AS
SELECT
  "tblStaticPages".id,
  "tblStaticPages".content,
  "tblStaticPages".name,
  "tblStaticPages".subpage,
  "tblStaticPages".updated
FROM
...
--
CREATE VIEW
  public."vwCourseAssistants" AS
SELECT
  sa."courseId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE("vwPeopleBase".name) || '###'::TEXT) || (COALESCE("vwPeopleBase".homepage, ''::CHARACTER VARYING))::TEXT)),
    '@@@'::TEXT
  ) AS assistants
FROM
  (
    ...
    JOIN (
      SELECT
        "brdgTeachers"."teacherId",
        "brdgTeachers"."peopleId",
        "brdgTeachers"."courseId",
        "brdgTeachers".type
      FROM
        public."brdgTeachers"
      WHERE
        (("brdgTeachers".type)::TEXT = 'a'::TEXT)
    ) sa USING ("peopleId")
  )
GROUP BY
  sa."courseId";
...
--
CREATE VIEW
  ivl."vwIVLTeachingCourses" AS
SELECT
  c."courseId",
  DATE_PART('year'::TEXT, c.year) AS YEAR,
  c."courseTitle",
  c."courseNumber",
  c."schoolDepartment",
  c.institution,
  ...
  (
    (
      (
        (
          public."tblCourses" c
          LEFT JOIN public."vwCourseTeachers" t USING ("courseId")
        )
        LEFT JOIN public."vwCourseAssistants" a USING ("courseId")
      )
      LEFT JOIN (
        SELECT
          "fncGetMedia"."mediumId",
          ...
          "fncGetMedia".rank
        FROM
          public."fncGetMedia" ('courses'::TEXT) "fncGetMedia" (
            "mediumId",
            id,
            TYPE,
            RANK
        ...
        WHERE
          ("fncGetMedia".rank = 1)
      ) s ON ((s.id = c."courseId"))
    )
    LEFT JOIN public."tblMedia" m USING ("mediumId")
  )
ORDER BY
  c."courseNumber",
  c.institution,
...
--
CREATE VIEW
  ivl."vwIVLTeachingEducMaterials" AS
SELECT
  "tblEducMaterials"."educMaterialId",
  "tblEducMaterials".title,
  "tblEducMaterials".description,
  "tblEducMaterials"."lastUpdated",
  s."mediumId",
  s."mediumEducMaterialId",
  s.rank,
  s."fileNameURL",
  s.type
FROM
  (
    public."tblEducMaterials"
    LEFT JOIN (
      SELECT
        "brdgMediaEducMaterials"."mediumId",
        "brdgMediaEducMaterials"."mediumEducMaterialId",
        "brdgMediaEducMaterials"."educMaterialId",
        "brdgMediaEducMaterials".rank,
        "tblMedia"."fileNameURL",
        "tblMedia".type
      FROM
        (
          public."brdgMediaEducMaterials"
          JOIN public."tblMedia" USING ("mediumId")
        )
    ) s USING ("educMaterialId")
  )
ORDER BY
  "tblEducMaterials"."lastUpdated" DESC;
...
--
CREATE VIEW
  ivl."vwIVLTeachingEducPresentations" AS
SELECT
  p."presentationId",
  p.title AS presentationtitle,
  p."additionalInfo",
  ...
  DATE_PART('year'::TEXT, pe.date) AS date,
  pe.date AS exactdate,
  ce.title AS eventstitle,
  ce."fileNameURL" AS eventurl,
  pep.presenters,
  l.city,
  l."StateProvince",
  t."fileNameURL",
      ...
      (
        (
          (
            ...
            LEFT JOIN public."vwEventsURL" ce USING ("eventId")
          )
          LEFT JOIN public."vwPresentationEventPresenters" pep USING ("presentationEventId")
        )
        LEFT JOIN public."fncGetLocations" ('events'::TEXT) l (
...
--
CREATE VIEW
  public."vwTutorialEventPresenters" AS
SELECT
  p."tutorialEventId",
  ARRAY_TO_STRING(
    public.array_accum (((COALESCE(b.name, ''::TEXT) || '###'::TEXT) || (COALESCE(b.homepage, ''::CHARACTER VARYING))::TEXT)),
    ...
    (
      (
        (
          (
            (
              public."tblTutorials" p
              LEFT JOIN public."brdgTutorialsEvents" pe USING ("tutorialId")
            )
            LEFT JOIN public."tblCallsEvents" ce USING ("eventId")
          )
          LEFT JOIN public."vwTutorialEventPresenters" pep USING ("tutorialEventId")
        )
        LEFT JOIN public."fncGetFullLocations" ('events'::TEXT) l (
          id,
          "locationId",
          TYPE,
          city,
      ...
      LEFT JOIN (
        SELECT
          ...
          "fncGetMedia".rank
        FROM
          public."fncGetMedia" ('tutorials'::TEXT) "fncGetMedia" (
            "mediumId",
            id,
            TYPE,
            RANK
...
SELECT
  "tblVideos".id,
  "tblVideos".title,
  "tblVideos".description,
  "tblVideos".video_type,
  "tblVideos".link,
  "tblVideos".pub_date,
  "tblVideos".embedded_video_code
FROM
  public."tblVideos";
...
--
CREATE VIEW
  ivl."vwIVLVisualizations" AS
SELECT
  "tblVisualizations".id,
  "tblVisualizations".title,
  "tblVisualizations".description,
  "tblVisualizations".media_type,
  "tblVisualizations".media_url,
  "tblVisualizations".link,
  "tblVisualizations".pub_date
FROM
  public."tblVisualizations";
...
--
CREATE VIEW
  ivl."vwIVLWorkshops" AS
SELECT
  "tblWorkshops".id,
  "tblWorkshops".title,
  "tblWorkshops".link,
  "tblWorkshops".start_date,
  "tblWorkshops".end_date,
  "tblWorkshops".city,
  "tblWorkshops".state,
  "tblWorkshops".country
FROM
  public."tblWorkshops";
...
--
CREATE VIEW
  public."vwAuthorRankedWithContact" AS
SELECT
  b."authorEditorId",
  b."peopleId",
  p."fName",
...
FROM
  public."brdgAuthorsEditors" b,
  public."tblPeople" p,
  public."vwPeopleBase" pb
WHERE
  (
    (
      (b.type ~~ 'a'::TEXT)
      AND (b."peopleId" = p."peopleId")
    )
    AND (b."peopleId" = pb."peopleId")
  )
ORDER BY
  b."publicationId" DESC,
  b.rank;
...
--
CREATE VIEW
  public."vwCoPIsCurrent" AS
SELECT DISTINCT
  b."peopleId"
FROM
  (
    public."brdgCoPIs" b
    JOIN public."tblGrants" g USING ("grantId")
  )
WHERE
  (g."endDate" > NOW())
ORDER BY
...
--
CREATE VIEW
  public."vwEditorRankedWithContact" AS
SELECT
  b."authorEditorId",
  b."peopleId",
  p."fName",
...
--
CREATE VIEW
  public."vwPeopleContactInformation" AS
SELECT
  "brdgPeopleContactInformation"."contactInformationId",
  "tblPeople"."peopleId",
  "tblPeople"."fName",
  "tblPeople"."mName",
  "tblPeople"."lName",
  "tblPeople".ivlweb,
  "tblPeople".cnsweb,
  "brdgPeopleContactInformation"."peopleContactInformationId",
  "brdgPeopleContactInformation".rank,
  "brdgPeopleContactInformation".type,
...
FROM
  (
    (
      public."tblPeople"
      JOIN public."brdgPeopleContactInformation" USING ("peopleId")
    )
    JOIN public."tblContactInformation" USING ("contactInformationId")
  );
...
--
CREATE VIEW
  public."vwPeopleResidence" AS
SELECT
  "vwPeopleBase"."peopleId",
  "vwPeopleBase".name,
  "tblLocations"."addrLine1",
  "tblLocations"."addrLine2",
  "tblLocations".city,
...
FROM
  (
    public."vwPeopleBase"
    LEFT JOIN (
      public."brdgResidence"
      LEFT JOIN public."tblLocations" USING ("locationId")
    ) USING ("peopleId")
  );
...
FROM
  (
    ...
    JOIN public."tblResearch" r USING ("researchId")
  )
WHERE
  (
    ((b.type)::TEXT = '010'::TEXT)
...
--
CREATE VIEW
  TEMP."vwDebugBryanPresentations" AS
SELECT
  b."mediumId",
  b."mediumPresentationId",
  b.rank,
  m."fileNameURL",
...
FROM
  (
    (
      public."brdgMediaPresentations" b
      JOIN public."tblMedia" m USING ("mediumId")
    )
    JOIN public."tblPresentations" p USING ("presentationId")
  );
